...
class Listing
  ...
  attr_reader :listing_id, :name, :free_date , :host_id, :price, :description

  def initialize(listing_id:, name:, free_date:, host_id:, price:, description:)
  ...
  def self.create(name:, free_date:, host_id:, price:, description:)
    result = DatabaseConnection.query("INSERT INTO listings (name, free_date, price, description)
    VALUES ('#{name}', '#{free_date}', '#{host_id}', '#{price}', '#{description}') RETURNING listing_id, name, free_date, host_id, price, description;")
